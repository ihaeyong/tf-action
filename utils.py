...
def evaluation_last(m_valid, sess, dataset, batch_size=1):
    list=np.zeros((2,3782))
    total_accuracy = 0
    num_test = 0
    count=0 
    count1=0
    ...
    while True:
        ...
        if dataset.epoch == 1:
            ...
            inputs, labels, x_name, y_name = dataset.next_batch_last(batch_size)
            x_name_pre=x_name[0]
            x_name_1=x_name_pre.split('/')[-1]
            x_name_1=x_name_1.split('.')[0]
            x_name_a=x_name_1.split('_')[1]
            x_name_b=x_name_1.split('_')[2]
            x_name_c=x_name_1.split('_')[3]

            x_name_total=str('v_'+x_name_a+'_'+x_name_b+'_'+x_name_c)
            if (count==0) :           
             x_name_buffer=x_name_total			

            count=1
            if not (x_name_buffer==x_name_total):
             count1+=1
             x_name_buffer=x_name_total
            print(x_name_total)			
            feed_dict = {m_valid.inputs: inputs, m_valid.labels: labels,
                         m_valid.keep_probability: 1}
            _, accuracy = sess.run([m_valid.train_op, m_valid.accuracy],
                                   feed_dict=feed_dict)

            list[0,count1]+=(1-accuracy)	
            list[1,count1]+=1	
            print(count1)
            if (count1==3781):
             break			
    tmp = (list[0]/(list[1]))>0.49
    tmp=tmp.astype(int)	
    kkk=np.mean(tmp)
    print(kkk)			
    return kkk
...
def evaluation(m_valid, sess, dataset, batch_size=512):
    ...
    return total_accuracy/num_test
